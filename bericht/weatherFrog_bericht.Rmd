...
```{r kNN_dist, echo = FALSE, fig.cap="\\label{fig:kNN_dist}10-NN Distanz mit Punkt der maximalen Wölbung", fig.align="center", out.width="85%"}
...
```{r mean_mslp_boxplot, echo = FALSE, fig.cap="\\label{fig:mean_mslp_boxplot}Verteilung der Variable Mittelwert des Luftdrucks in jedem Cluster", fig.align="center", out.width="70%"}
include_graphics("assets/mean_mslp_boxplot.png")
```

```{r mean_geopot_boxplot, echo = FALSE, fig.cap="\\label{fig:mean_geopot_boxplot}Verteilung der Variable Mittelwert des Geopotentials in jedem Cluster", fig.align="center", out.width="70%"}
...
```{r mslp_measurepoints, echo = FALSE, fig.cap="\\label{fig:mslp_measurepoints}Räumliche Verteilung des Luftdrucks je Cluster. Für jeden der 160 Standorte wurde das arithmetrische Mittel der Variable Geopotential über alle Tage im Zeitraum von 1971-2000 berechnet.", fig.align="center", out.width="90%"}
...
```{r geopot_measurepoints, echo = FALSE, fig.cap="\\label{fig:geopot_measurepoints}Räumliche Verteilung des Geopotentials je Cluster. Für jeden der 160 Standorte wurde das arithmetrische Mittel der Variable Geopotential über alle Tage im Zeitraum von 1971-2000 berechnet.", fig.align="center", out.width= "90%"}
...
Hier ergibt sich mit $k = 5$ Clustern ein Silhouettenkoeffizient von $s = 0.137$ sowie einem Timeline-Score von $TLS = 0.4118$ (siehe Abb. \ref{fig:TL_filter+exa}). Zudem lässt sich ein $HB_{diff}$ von $0.4122$ beobachten (siehe Abb. \ref{fig:mosaic_filter+exa}).
...
Anhand der geringen Clusterbewertungskriterien des Filter-Ansatz' mit Geopotential (siehe Absatz \ref{kriterien-filtern}) sowie den über die Cluster gemittelten Tagesbilder des Clusterns der extrahierten Daten mit PAM, bei dem sich die Bilder des Geopotentials räumlich sehr wenig unterscheiden (Abb. \ref{fig:geopot_measurepoints}), lässt sich die Vermutung aufstellen, dass die Ausprägungen des Geopotentials, zumindest so wie sie vorliegen, räumlich kaum Unterschiede aufweisen und somit sich zwischen Tagen keine verschiedenen Strukturen finden lassen (siehe Abb. \ref{fig:geopot_verID}). 
...
Insgesamt lässt sich festhalten, dass hier vorgestellte Clusterverfahren in Bezug auf den Silhouettenwert Strukturen bis zu ca. $s = 0.15$ aufweisen. Der zeitliche Verlauf der Tageseinteilung der Clusterergebnisse erreicht dabei teilweise annähernd sinnvolle Stukturen mit *CWL* zusammenhängender Tage einer als sinnvoll angenommenen Länge. Und das, obwohl die zeitliche Komponente der Daten in der Clusterbildung nicht berücksichtigt wird. Letzteres wurde als einer der eventuell größeren Mängel der vorgestellten Methoden erkannt.
...
Im Kapitel \ref{diskussion-und-ausblick} werden zwar einige vielversprechende Ansätze beschrieben, die Methoden zu verbessern bzw. auszubauen. Diese benötigen aber eventuell eine tiefgründige Analyse zur fachlichen Sinnhaftigkeit vor einer Implementation. 
