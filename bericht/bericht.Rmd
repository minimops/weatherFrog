...
data.table(
  ...
  Kurzform = c("GWL", "CLARA", "Mslp", "Geopotential", "$\\sum$", "PAM", "DBSCAN", "SCAPOI", "CWL", "TLS", "${HB}_{diff}$", "PC / PCA", "$k$")) %>%
  arrange(Ausdruck) %>%
...
kbl(gwl, booktabs = TRUE, format =  "latex", escape = FALSE, label = "gwl", caption = "Großwetterlagen nach Hess und Brezowsky") %>%
...
Seit 1900 wird viermal täglich an 160 Standorten der Luftdruck in Pascal auf Meeresspiegelhöhe (Mslp) und das Geopotential auf 500 hPa in $\frac{m^2}{s^2}$ erhoben. Im Folgenden wird für Visualisierungen der Luftdruck in hPa und das Geopotential für .... dargestellt. Die Standorte befinden sich in einem 8x20 Grid über Europa und Teilen des Atlantik, wie in Abb. \ref{fig:locations} dargestellt ist.
...
Dieser Ansatz liefert für beide Parameter kombiniert einen *Silhouettenkoeffizient* von $0.0832$ und einen *TLS* von $-0.1527$. Dabei wurden die Distanzmatrizen des Filterns der beiden Parameter addiert und damit eine Clusteranalyse, wie in Abschnitt [Partitioning Around Medoids] (*PAM*) beschrieben, durchgeführt.

Nach Parametern getrennte Analysen weisen je einen höheren *Silhouettenkoeffizient* auf, aber einen geringeren *TLS*. (Mslp: $s = 0.1369$, $TLS = -0.2027$ ; Geopot: $s = 0.1541$, $TLS = -0.3898$)
...
```{r tlMultiplied, echo = FALSE, fig.cap = "\\label{fig:tlMultiplied}Timeline für Clusterergebnis mit extrahierten Variablen mit $k = 6$", fig.align="center", out.width = "80%"}
...
Des Weiteren wurde auch überlegt, den Datensatz in Sommer- und Winterdaten aufzuteilen und diese getrennt zu clustern (@article_james). Da ein Clustering für 30 Jahre rechentechnisch aufwendig ist, wurden zunächst vier 5-Jahres-Perioden betrachtet, von 1971-1975, 1984-1988, 1996-2000 und 2006-2010 und bewertet. 

Damit die Ergebnisse vergleichbar sind, wurde Sommer- und Winterzeit zusammen sowie getrennt geclustert. Das Clustering wurde mit PAM und der Manhattan-Distanz durchgeführt. Die Optimale Anzahl an Cluster war für alle Versuche hier $k = 5$. \textcolor{red}{brauchen wir plots?}

Der Silhouettenkoeffizient hat hierbei für das Clustern der Sommer- und Wintertage zusammen den höchsten Wert mit $s = 0.1148$. Der Silhouettenkoeffizient für die Sommertage beträgt 0.0976 und für die Wintertage ist $s = 0.0945$.

Der Timeline-Score für das Clustern beider Jahreszeiten zusammen beträgt hier 0.2552, wohingegen $TLS_{Sommer} = 0.0199$ und $TLS_{Winter} = -0.0336$.

Die Werte für die Güte der Aufteilung der GWL in den Clustern liegt für das Cluster beider Jahrezeiten zusammen bei 0.4739. Dieser Wert ist für Clusterings mit jeweils Sommer- und Wintertagen getrennt etwas höher mit $HB_{diff, Sommer} = 0.4916$ und $HB_{diff, Winter} = 0.5126$.

Die Bewertungskriterien Silhouettenkoeffizient und $HB_{diff} weisen hier ähnliche Ergebnisse auf. Es wurde sich letztendlich gegen ein Clustering mit getrennten Jahreszeiten entschieden, da der Silhouettenkoeffizient dafür niedriger wäre.
...
\textcolor{red}{HB_diff mit einbringen}
...
Die Methode, wie in Absatz [Filter Ansatz] beschrieben, weist in dessen Status Quo womöglich einen zu hohen Informationsverlust auf, um alleinig angewendet zu werden. Intuitiv lässt sich vermuten, dass ein Zusammenführen mit dem im Absatz [Clustern mit extrahierten Daten] beschriebenen Verfahren zu einer insgesamten Verbesserung führen könnte. Folgendes kann durch Addieren der Distanzmetriken vor dem Clustern auf Tagesebene erreicht werden. Die darauf folgende Clusteranalyse wurde hier auch mit *PAM* durchgeführt.
...
Allerdings fällt hierbei die Clusteranzahl von $k = 6$ auf $k = 5$, was den Vergleich des $TLS$ sowie des $HB_{diff}$ wie in Absatz [Bewertungkriterien] beschrieben, problematisiert.
...
kbl(dt4, booktabs = TRUE, format =  "latex", escape = FALSE, label = "Gewichtung", caption = "R Libraries") %>%
  kable_styling(latex_options = "striped", full_width = FALSE)%>%
