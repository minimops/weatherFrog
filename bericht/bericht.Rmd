...
```{r mosaic_seasons, echo = FALSE, fig.cap="\\label{fig:mosaic_seasons}Gestapeltes Balkendiagramm, der den relativen Anteil an Winter- und Sommertagen je Cluster abbildet", fig.align="center", out.width="80%"}
...
Es wurde auch in Betracht gezogen, eine allgemeine Saisonbereinigung durchzuführen. Dabei wurde ursprünglich angedacht, die Saisonbereinigung anhand der vier Jahreszeiten, Winter, Frühling, Sommer und Herbst durchzuführen. In Abbildungen \ref{fig:werteMslp} und \ref{fig:werteGeopot} sind die Werte des Luftdrucks bzw. Geopotentials aufgeteilt nach den Jahrezeiten dargestellt. Vor allem der Luftdruck zeigt keine deutlichen Unterschiede in Bezug auf die Saison. Der Median liegt für alle bei etwas unter $1020 hPa$. Nur die Streuung fällt im Sommer etwas geringer aus als in den anderen Jahrezeiten. Wie im Artikel von @article_james vorgeschlagen, erscheint demnach eine Auteilung in 2 Jahreszeiten sinnvoller. Da in den Clustern 1, 2 und 3 vorwiegend Wintertage und in den Clustern 4, 5 und 6 hauptsächlich Sommertage sind, siehe Abschnitt \ref{verhaeltnis-von-sommer--und-wintertagen-in-den-clustern}, erscheint eine Analyse mit zwei getrennten Jahreszeiten hier plausibel.

```{r werteMslp, echo = FALSE, fig.cap="\\label{fig:werteMslp}Werte des Luftdrucks der Jahre 1971 - 2000 aufgeteilt nach 4 Jahreszeiten", fig.align="center", out.width="80%"}
include_graphics("assets/seasonsMslp.png")
```

```{r werteGeopot, echo = FALSE, fig.cap="\\label{fig:werteGeopot}Werte des Geopotentials der Jahre 1971 - 2000 aufgeteilt nach 4 Jahreszeiten", fig.align="center", out.width="80%"}
...
Insgesammt lässt sich festhalten, dass hier vorgestellte Clusterverfahren in Bezug auf den Silhouettenwert Strukturen bis zu ca. $s = 0.15$ aufweisen. Der zeitliche Verlauf der Tageseinteilung der Clusterergebnisse erreicht dabei teilweise annähernd sinnvolle Stukturen mit *CWL* zusammenhängender Tage einer als sinnvoll angenommenen Länge, trotzdem, dass die zeitliche Komponente der Daten in der Clusterbildung nicht berücksichtig wird. Letzteres wurde als einer der eventuell größeren Mängel der vorgestellten Methoden erkannt.

Die *GWL* nach Hess und Brezowski finden sich jeweils meistens in allen Ergebnisclustern wieder. Zum Teil heben sich dennoch *GWL* hervor, die mit einem größeren Anteil einem bestimmten Cluster zugeteilt werden können. Eine besonders saubere Teilung der 29 *GWL* in allgemeinere Gruppen lässt allerdings nicht feststellen.

Im Kapitel \ref{diskussion-und-ausblick} werden zwar einige vielversprechende Ansätze beschrieben, die Methoden zu verbessern bzw. auszubauen, diese benötigen aber eventuell eine tiefgründige Analyse zur fachlichen Sinnhaftigkeit vor der Implementation. 
...
```{r Funktionen und Pakete, echo=FALSE, fig.cap="\\label{tab:libraries}Benutzte Pakete in R der im Bericht beschriebenen Clusterverfahren", }

dt4 <- data.table(
  Verfahren = c("CLARA", "PAM", "k-Means", "DBSCAN", "kNN-Distanz", "FUZZY"),
  Funktion = c("clara()", "pam()", "kmeans()", "dbscan()", "kNNdist()", "cmeans()")
)
