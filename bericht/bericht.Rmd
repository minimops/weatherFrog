...
kbl(gwl, booktabs = TRUE, format =  "latex", escape = FALSE, label = "gwl", caption = "Großwetterlagen nach Hess und Brezowsky", linesep  = "") %>%
...
dt2 <- data.frame(
  ...
  Gewichte = c("$1/3$", "$1/3$", "$1/3$", "$1/6$", "$1/6$", "1/6", "1/9", "1/9")  
  ) 

kbl(dt2, booktabs = TRUE, format =  "latex", escape = FALSE, label = "Gewichtung", caption = "Gewichte für Variablen", align = "l", linesep  = "") %>%
...
Das endgültig gewählte Methode wurde mit der Distanzmetrik Manhattan und dem Clusteralgorithmus *PAM* anhand des extrahierten Datensatzes erzielt. Es wurden anfangs aber verschiedene Methodiken ausprobiert und anhand der definierten Bewertungskriterien beurteilt. Dabei wurden zuerst die Daten von fünf Jahren verwendet, da 30 Jahre rechentechnisch sehr teuer waren. Die Methoden, die am besten abschnitten, wurden dann anhand von 30 Jahren beurteilt. Im Folgenden werden weitere Versuche kurz dargestellt und zusammengefasst.
...
Zum einen wurde ein Clustering mit dem Algorithmus Clustering Large Applications (*CLARA*) durchgeführt. Dieser basiert auf dem K-Medoids-Ansatz, ist aber speziell für Datensätze, die sehr viele Beobachtungen enthalten, z.B. mehrere tausend Objekte. Dieser Algorithmus wird von @clara eingeführt und erklärt.
...
kbl(dt3, booktabs = TRUE, format =  "latex", escape = FALSE, label = "Gewichtung", caption = "Versuche von verschiedenen Methoden für 5 Jahre", linesep  = "") %>%
  kable_styling(latex_options = c("hold_position","striped"), full_width = TRUE)%>%
...
```{r werteMslp, echo = FALSE, fig.cap="\\label{fig:werteMslp}Werte des Luftdrucks aufgeteilt nach 4 Jahreszeiten", fig.align="center", out.width="100%"}
include_graphics("assets/MslpRangeJahreszeiten.jpeg")
```

```{r werteGeopot, echo = FALSE, fig.cap="\\label{fig:werteGeopot}Werte des Geopotential aufgeteilt nach 4 Jahreszeiten", fig.align="center", out.width="100%"}
include_graphics("assets/GeopotentailRangeJahreszeiten.jpeg")
```
...
kbl(dt4, booktabs = TRUE, format =  "latex", escape = FALSE, label = "Gewichtung", caption = "R Libraries", linesep  = "") %>%
  kable_styling(latex_options = "striped", full_width = FALSE)%>%
  pack_rows("cluster", 1, 3) %>%
