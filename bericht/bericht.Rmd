...
Der Klimawandel und seine Auswirkungen sind in den letzten Jahrzenten spürbar geworden. Immer häufiger kommt zu extremen Wetterereignissen wie langanhaltende Hitzewellen, Überschwemmungen Dürre und (Wald-)Brände. Durch den Klimawandel ändert sich auch die Häufigkeit und Länge der auftretenden Großwetterlagen.
Lang anhaltende GWL sind problematisch, da sie zu  Wetteranomalien führen und Variablen wie Temperatur, Regen, Wind und Wetter beeinflussen, welches sich wiederum auf das Klima auswirkt (@article_james).
Magdalena Mittermeier vom Department für Geographie und Maximilian Weigert vom Statistischen Institut der LMU untersuchen, wie sich das Auftreten verschiedener Großwetterlagen (GWL) unter dem Einfluss des Klimawandels verändert.
Im Rahmen des Statistischen Praktikums wird dieses Projekt unterstützt, indem explorativ versucht wird, Tage anhand von beobachteten Wetterdaten in Cluster aufzuteilen.
...
Es stellt sich nun die Frage, ob sich anhand der Variablen Luftdruck auf Meeresspiegel und Geopotential die  einzelnen Tage sinnvoll in Cluster einteilen lassen, ohne die Informationen der GWL zu benutzen.
+ Vorgaben
...
### Kriterien
  ...
  Nach Parametern getrennte Analysen weisen je einen höheren *Silhouettenkoeffizient* auf, aber einen geringeren *TLS*. (Mslp: $sil = 0.1369$, $TLS = -0.2027$ ; Geopot: $sil = 0.1541$, $TLS = -0.3898$)
...
Obwohl die Entscheidung auf PAM als Clusteralgorithmus gefallen ist, wurde auch ein K-Means Clustering mit Hilfe der Funktion kmeans() in R aus dem Package "stats" für den Zeitraum 1971-2000 durchgeführt. Diese benutzt die Euklidische Distanz und für die Berechnung der Distanzmatrix wurden diesselben Variablen und Gewichte herangezogen, wie in Abschnitt 2.3 erklärt. 
...
\textcolor{red}{needs to be changed angain :/}
...
Es wurde auch in Betracht gezogen, eine Saisonbereinigung durchzuführen. Dabei wurde ursprünglich angedacht, die Saisonbereinigung anhand der vier Saisons, Winter, Frühling, Sommer und Herbst durchzuführen. Nachdem aber Abbildungen \ref{fig:werteMslp} und \ref{fig:werteGeopot} betrachtet wurden, ist die Entscheidung gegen eine Saisonbereinigung getroffen worden. \textcolor{red}{das überhaupt rein?}
...
### Aufteilen nach Saison vor Clustering

Des Weiteren wurde auch überlegt, den Datensatz in Sommer- und Winterdaten aufzuteilen und diese getrennt zu clustern (@article_james). Da ein Clustering für 30 Jahre rechentechnisch aufwendig ist, wurden zunächst vier 5-Jahres-Perioden betrachtet, von 1971-1975, 1984-1988, 1996-2000 und 2006-2010 und bewertet. \textcolor{red}{reinbringen, dass Variablen anders sind?}
...
Der Vergleich des Timeline-Scores erweist sich hier als problematisch. Dieser wird gebildet, indem die Anzahl der Tage mit bestimmten Werten einer Funktion multipliziert werden. Da sonst für vorherige Analysen 30 Jahre betrachtet wurden und dies generell eine höhere Anzahl an Tagen mit sich bringt, können die Folgenden Werte nicht mit den bereits vorgestellten verglichen werden. Auch ein Vergleich zwischen Sommer- und Wintertage zusammen bzw. getrennt, ist hier schierig, da bei beiden Zeiten zusammen jeweils vier Mal 5 Jahre betrachtet werden. Für das Cluster der Sommer- und Wintertage getrennt werden hingegen nur ca. 2.5 Jahren betrachtet.
...
Die Bewertungskriterien Silhouettenkoeffizient und Mosaikwert weisen hier ähnliche Ergebnisse auf. Es wurde sich letztendlich gegen ein Clustering mit getrennten Jahreszeiten entschieden, da der Silhouettenkoeffizient dafür niedriger wäre.
...
 
  ...
  Hier ergibt sich mit $k = 5$ Clustern ein *Silhouettenkoeffizient* von $sil = 0.137$ sowie einem *Timeline-Score* von $TLS = 0.4118$. (siehe Abb. \ref{fig:TL_filter+exa})
