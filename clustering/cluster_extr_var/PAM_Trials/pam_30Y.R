...
scaled.pca <- scaleNweight(copy(data.pca))
...
# but apparantly, weights in daisy are only used if metric "gower" is selcted
...
diss.pam.manhat.30.all.weighted <- daisy(scaleNweight(copy(data), weight = TRUE)[, 2:ncol(data)], 
                                          metric = "manhattan") 
saveRDS(diss.pam.manhat.30.all.weighted, "Data/diss.pam.manhat.30.all.weighted.rds")
...
# 2. Timeline
dat.manhat.30.weighted <- copy(data)[, cluster := cluster.manhat.30.weighted]
Cl.timeline(copy(dat.manhat.30.weighted), seperated = TRUE)

# 3. Manova
manova.manhat.30.weighted <- manova.fun(copy(data), cluster.manhat.30.weighted)

# 4. Mosaic
mosaic(copy(data), cluster.manhat.30.weighted, title = "PAM WITH MANHAT - 30 YEARS - weighted")
...
diss.pam.manhat.30.allpca <- dissimilarityPAM(scaleNweight(copy(data.pca)), 
                                              dist = FALSE, 
                                              metric = "manhattan",
                                              weights = c(rep(c(1/9, 1/9, 1/6, 1/6, 1/18, 1/18, 1/9, 1/9, 1/9), 2), 
...
diss.pam.manhat.30.allpca.weighted <- dissimilarityPAM(scaleNweight(copy(data.pca), 
                                                                    weight = TRUE),
...
weighted <- scaleNweight(copy(data.pca), weight = TRUE, 
                         weights = c(rep(c(1/9, 1/9, 1/6, 1/6, 1/18, 1/18, 1/9, 1/9, 1/9), 2), 
                                     rep(1/6, 12), rep(1/21, 21)))
...
diss.pam.gower.30.allpca <- dissimilarityPAM(scaleNweight(copy(data.pca)), 
                                              weights = c(rep(c(1/9, 1/9, 1/6, 1/6, 1/18, 1/18, 1/9, 1/9, 1/9), 2), 
                                                          rep(1/6, 12), rep(1/21, 21)),
                                              dist = FALSE, metric = "gower")

diss.pam.gower.30.allpca <- daisy(scaled.pca[, 2:ncol(scaled.pca)], metric = "gower", 
                               weights = c(rep(c(1/9, 1/9, 1/6, 1/6, 1/18, 1/18, 1/9, 1/9, 1/9), 2), 
                                           rep(1/6, 12), rep(1/21, 21)))
